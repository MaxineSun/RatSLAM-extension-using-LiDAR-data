...
namespace ratslam
{
  ...
  LocalViewMatch::LocalViewMatch(ptree settings)
  {
    ...
    //templates is a vector<VisualTemplate>
    templates.reserve(10000);
    //current view is a vector<double>
  ...
  // main function for the matching, is called in the callback from the main_lv subscriber
  // hence, it is executed every time the subscriber receives a message, i.e., a new image
