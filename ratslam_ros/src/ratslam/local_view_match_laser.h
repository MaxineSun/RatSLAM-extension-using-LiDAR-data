...
public:
  ...
  void on_scan(sensor_msgs::PointCloud2 view_cloud);
  void on_scan(const float *ranges, unsigned int laser_size, float range_max);
  ...
  template<typename Archive>
    void serialize(Archive& ar, const unsigned int version)
    {
      ...
      ar & VT_NORMALISATION;
      ar & VT_PANORAMIC;
      ar & VT_PANORAMIC_LASER;
      ar & LASER_AVG;
      ar & LASER_VG;
      ar & LASER_SIZE;
      ar & LASER_TEMPLATE_SIZE;
      ar & LASER_RANGE_MAX;
      ar & VT_MATCH_THRESHOLD_LASER;
      ...
      ar & prev_vt;
      ar & vt_relative_rad;
      ar & current_view_laser;
      ar & current_mean_laser;
      ar & vt_error_laser;
      ar & vt_relative_rad_laser;
...
private:
  ...
  void convert_view_to_view_template_avg();
  void convert_view_to_view_template_vg();
  ...
  void compare_avg(double &vt_err_laser, unsigned int &vt_match_id_laser);
  void compare_fusion(double &vt_err_laser, unsigned int &vt_match_id_laser, unsigned int vt_matched_camera);
  ...
  int LASER_VG;
  int LASER_SIZE;
  int LASER_TEMPLATE_SIZE;
  double LASER_RANGE_MAX;
  double VT_MATCH_THRESHOLD_LASER;
  int VT_PANORAMIC_LASER;

  std::vector<VisualTemplate> templates;
  std::vector<VisualTemplateVG> templates_vg;
  std::vector<double> current_view;
  std::vector<double> current_view_laser;
  pcl::PointCloud<pcl::PointXYZ>::Ptr current_view_vg;
  ...
  double current_mean_laser;
  double vt_error_laser;
  double vt_relative_rad_laser;

  sensor_msgs::PointCloud2 view_cloud;
  const float *ranges;
